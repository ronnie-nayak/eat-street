...
export function Sections({ title, image }: { title: string, image: string }) {
  return (
    <div className=" h-[23.75vh] md:h-[40vh]  w-full px-4 py-8 m-2 rounded-3xl xl:px-8 xl:py-20 hover:-translate-y-2 transition-all duration-500" style={{
      ...
      <h1
        className=" text-[2vw] leading-none text-[#243F2F] mb-6 whitespace-pre-wrap ">
        {title}
      </h1>
  ...
  return (
    <div>
      <div style={{
        ...
        <h1
          className="text-[2vw]  leading-none text-[#EBFFDC] ">
          {title}
        </h1>
        <button className="bg-white  p-4 rounded-full mb-4 hover:text-white transition-all duration-300 hover:bg-limeGreen">
          Shop Now
...
export function Sections3({ title, image, discount }: { title: string, image: string, discount: number }) {
  return (
    <div style={{
      ...
      <button className="bg-white text-[#00AA63] text-[1.5vw]  px-5 py-1 rounded-full mb-4">
        -{discount}%
      </button>
      <h1
        className="text-[2vw]  leading-none text-[#243F2F] ">
        {title}
      </h1>
    </div>
  )
  ...
  return (
    <div style={{
      ...
      <div className="flex flex-col items-center justify-center gap-4 self-center">
        <h2 className=" text-[#D4FFFF] text-[2vw]">
          Order from our Apps and Get Free Delivery
        </h2>
        <small className="text-[#D4FFFF] text-[1vw]  ">
...
export function Info({ image, first, second }: { image: string, first: string, second: string }) {
  return (
    <div>
      ...
      <h4 className="font-normal text-[1vw]">
        {second}
      </h4>
    </div>
  )
