...
export function Sections({ title, image, path }: { title: string, image: string, path: string }) {
  return (
    <div className=" h-[23.75vh] md:h-[40vh]  w-full px-4 py-8 m-2 rounded-3xl xl:px-8 xl:py-20 hover:-translate-y-2 transition-all duration-500" style={{
      ...
      <h1
        className=" sm:text-[2vw] leading-none text-[#243F2F] mb-6 whitespace-pre-wrap ">
        {title}
      </h1>
      <Link href={path} className="bg-white  p-4 rounded-full mb-4 hover:text-white transition-all duration-300 hover:bg-limeGreen">
        Shop Now
      </Link>
    </div>
  )
  ...
  return (
    <div>
      <div style={{
        ...
        <h1
          className="sm:text-[2vw]  leading-none text-[#EBFFDC] ">
          {title}
        </h1>
        <Link href={"/homepage/baking"} className="bg-white  p-4 rounded-full mb-4 hover:text-white transition-all duration-300 hover:bg-limeGreen">
          Shop Now
        </Link>
      </div>
    </div >
...
export function Sections3({ title, image, discount }: { title: string, image: string, discount: number }) {
  return (
    <div style={{
      ...
      <h1
        className="sm:text-[2vw]  leading-none text-[#243F2F] ">
        {title}
      </h1>
    </div>
  )
  ...
  return (
    <div style={{
      ...
      <div className="flex flex-col gap-4 text-[#D4FFFF] items-center justify-center">
        ...
        <Link href={"/homepage/carts"} className="bg-white text-[#276153] p-4 rounded-full mb-4 hover:text-white transition-all duration-300 hover:bg-limeGreen">
          Read More
        </Link>
      </div>
    </div>
...
export function Info({ image, first, second }: { image: string, first: string, second: string }) {
  return (
    <div>
      <img src={image} />
      <h3 className="sm:text-[1vw]">
        {first}
      </h3>
      <h4 className="font-normal sm:text-[1vw]">
        {second}
      </h4>
    </div>
  )
